...
const { getBuffer, getGroupAdmins, getRandom, h2k, isUrl, Json, runtime, sleep, fetchJson } = require('./lib/functions') const fs = require('fs') const P = require('pino') const config = require('./config') const qrcode = require('qrcode-terminal') const util = require('util') const { sms, downloadMediaMessage } = require('./lib/msg') const axios = require('axios') const { File } = require('megajs') const prefix = '.'

const ownerNumber = ['94768698018']

let replyMap = new Map()

if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) { if (!config.SESSION_ID) return console.log('Please add your session to SESSION_ID env !!') const sessdata = config.SESSION_ID const filer = File.fromURL(https://mega.nz/file/${sessdata}) filer.download((err, data) => { if (err) throw err fs.writeFile(__dirname + '/auth_info_baileys/creds.json', data, () => { console.log("Session downloaded ✅") }) }) }
...
if (replyMap.has(mek.message?.extendedTextMessage?.contextInfo?.stanzaId)) {
  const { callback } = replyMap.get(mek.message.extendedTextMessage.contextInfo.stanzaId)
  return callback(m)
}
...
app.get("/", (req, res) => { res.send("hey, bot started✅") }) app.listen(port, () => console.log(Server listening on port http://localhost:${port})) setTimeout(() => { connectToWA() }, 4000)
