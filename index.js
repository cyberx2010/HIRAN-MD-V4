...
async function connectToWA() {
  ...
  // Add replyList method
  conn.replyList = async (jid, message, quoted = {}) => {
    const msg = {
      listMessage: {
        title: message.title || 'Select an option',
    ...
    // Store the callback for handling replies
    if (message.callback) {
      conn.addReplyTracker(sentMsg.key.id, message.callback);
    }
...
╰━━━━━━━━━━━━━━━━━━━╯`;
  ...
  conn.ev.on('messages.upsert', async (mek) => {
    ...
    if (replyMap.has(stanzaId)) {
      ...
      const responseText = (mek.message?.conversation || mek.message?.extendedTextMessage?.text || '').trim();
      return callback(m, responseText, { from, reply, quoted, args, q });
    }
