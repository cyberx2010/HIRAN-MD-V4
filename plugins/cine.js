        ...
        conn.addReplyTracker(sentMsg.key.id, async (mek, messageType) => {
            ...
            try {
                ...
                if (!movieData.success) {
                    ...
                    return await reply(`*Error fetching movie details: ${movieData.error || 'Unknown error'}*`);
                }
                    ...
                    if (option === '1') {
                        ...
                        conn.addReplyTracker(sentDownloadMsg.key.id, async (mek, downloadMessageType) => {
                            ...
                            try {
                                ...
                                // Check if API call was successful
                                if (!movieLinkData.success) {
                                    console.error('API Error:', movieLinkData.error);
                                    return await reply(`*Error fetching download link: ${movieLinkData.error || 'Unknown error'}*`);
                                }
                                ...
                                await reply(`*Error fetching download link: ${error.message || 'Please try again.'}* Raw link: ${selectedLink.link}`);
                            }
                        });
                    } else if (option === '2') {
                        // Details option
                ...
                console.error('Error during movie selection:', error.message);
