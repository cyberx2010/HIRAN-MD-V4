const { cmd } = require('../command'); const yts = require('yt-search'); const ddownr = require('denethdev-ytmp3'); const fetch = require('node-fetch'); const defaultPreview = 'https://files.catbox.moe/16uz1x.jpg';

// SONG COMMAND cmd({ pattern: "song", desc: "Download songs.", category: "download", react: '🎧', filename: __filename }, async (m, ctx, quoted, { from, reply, q }) => { try { if (!q) return reply("Please Provide A Song Name or Url 🙄");
...
} catch (err) { console.error(err); reply("An Error Occurred While Processing Your Song Request 😔"); } });

// VIDEO COMMAND cmd({ pattern: "video", alias: ["mp4", "ytv"], react: "🎥", desc: "Download YouTube video", category: "download", filename: __filename }, async (m, ctx, quoted, { from, reply, q }) => { try { if (!q) return reply("Please provide a YouTube URL or video name 🙄");
...
} catch (err) { console.error(err); reply("An Error Occurred While Processing Your Video Request 😔"); } });
